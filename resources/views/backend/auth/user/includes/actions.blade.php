@if ($user->trashed() && $logged_in_user->hasAllAccess())
    <x-utils.form-button
        ...
        name="confirm-item"
    >
        @lang('translator.label_restore')
    </x-utils.form-button>

    @if (config('boilerplate.access.user.permanently_delete'))
        <x-utils.delete-button
            :href="route('admin.auth.user.permanently-delete', $user)"
            :text="__('translator.label_delete_permanently')" />
    @endif
@else
    @if ($logged_in_user->hasAllAccess())
        <a class="btn btn-light btn-sm" href="{{route('admin.auth.user.show', $user)}}"> @lang('translator.label_view') </a>
        <a class="btn btn-success btn-sm" href="{{route('admin.auth.user.edit', $user)}}"> @lang('translator.label_edit') </a>
    @endif
    ...
    @if (! $user->isActive())
        ...
        >
            @lang('translator.label_reactivate')
        </x-utils.form-button>
    @endif
        ...
        <div class="dropdown d-inline-block">
            ...
            <div class="dropdown-menu" aria-labelledby="moreMenuLink">
                <x-utils.link
                    ...
                    class="dropdown-item"
                    :text="__('translator.change_password')"
                ...
                @if ($user->id !== $logged_in_user->id && !$user->isMasterAdmin())
                    ...
                    @canBeImpersonated($user)
                        <x-utils.link
                            ...
                            class="dropdown-item"
                            :text="__('translator.login_as') .' ' . $user->name"
