...
<head>
    ...
    <link href="https://fonts.googleapis.com/css2?family=Raleway:wght@100;300;400;500;600;700;900&display=swap"
        rel="stylesheet">
    ...
    <div id="app">
        ...
        <div class="user-bar ">
           ...
           <li  class="{{ Route::currentRouteName() == 'frontend.auth.register' ? 'active' : '' }}"><a href="{{ route('frontend.auth.register') }}">Register</a></li> -->
                    <button class="log-in"
                        class="{{ Route::currentRouteName() == 'frontend.auth.login' ? 'active' : '' }}"><a
                            href="{{ route('frontend.auth.login') }}"
                            type="button">@lang('translator.log_in_button')</button>

                    <button class="sign-up"
                        class="{{ Route::currentRouteName() == 'frontend.auth.register' ? 'active' : '' }}"><a
                            href="{{ route('frontend.auth.register') }}"
                            type="button">@lang('translator.sign_up_button')</button>
                @endif
                @if (config('boilerplate.locale.status') && count(config('boilerplate.locale.languages')) > 1)
                    <li class="c-header-nav-item dropdown lang-list">
                        <x-utils.link :text="__(getLocaleName(app()->getLocale()))"
                            class="c-header-nav-link dropdown-toggle" id="navbarDropdownLanguageLink"
                ...
                @if (Auth::check())
                    @if ($logged_in_user->isAdmin())
                        <li class="c-header-nav-item dropdown ">
                            <x-utils.link class="dropdown-item admin-route" icon="c-icon  cil-account-logout"
                    ...
                    <li class="c-header-nav-item dropdown ">
                        <x-utils.link class="c-header-nav-link" data-toggle="dropdown" href="#" role="button"
                            ...
                            <x-slot name="text">
                                <div class="c-avatar">
                                    @inject('user', 'App\Models\User')
                                    <img class="c-avatar-img" src="{{ $user::returnAuthUserAvatar($logged_in_user) }}"
